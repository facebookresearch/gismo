...
class Im2Recipe(nn.Module):
    def __init__(self, 
        ...
        self.ingr_encoder = IngredientsEncoder(recipegen_args.embed_size, num_classes=ingr_vocab_size,
                                               dropout=recipegen_args.dropout, scale_grad=False)

        # recipe generator model
        self.recipe_gen = RecipeGenerator(recipegen_args, instr_vocab_size, maxrecipelen)


    def forward(self, img, recipe_gt, ingr_gt=None, use_ingr_pred=False, compute_losses=False, compute_predictions=False):
        ...
        if use_ingr_pred:
            ...
            ingr_losses, ingr_predictions = self.ingr_predictor(img_features, label_target=ingr_gt, 
            ...
            ingr_mask = mask_from_eos(ingr_predictions, eos_value=self.ingr_eos_value, mult_before=False)
            ...
            # get ingredients' mask
            ingr_mask = mask_from_eos(ingr_gt, eos_value=self.ingr_eos_value, mult_before=False)
            ingr_mask = ingr_mask.float().unsqueeze(1)    

        # generate recipe and compute losses if necessary
        loss, predictions = self.recipe_gen(img_features, ingr_features, ingr_mask, 
                                            recipe_gt, compute_losses=compute_losses, 
