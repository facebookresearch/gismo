...
def main(cfg: DictConfig) -> None:
    ...
    # checkpointing
    checkpoint_dir = os.path.join(cfg.checkpoint.dir, cfg.task+'-'+cfg.ingr_predictor.model)
    ...
    # model
    model = LitInverseCooking(task=cfg.task,
    ...
    # checkpointing
    checkpoint_callback = ModelCheckpoint(monitor=monitor_metric,
    ...
    trainer = pl.Trainer(
        ...
        precision=32,
        callbacks=[checkpoint_callback, early_stop_callback],  # need to overwrite ModelCheckpoint callback? check loader/iterator state
        logger=tb_logger,
