...
def schedule_jobs(configurations: List[Config], mode: str) -> None:
    copy_source_code_to_cwd()  # Because Hydra create a new running folder
    for config in configurations:
        if config.slurm.partition == "local":
            _schedule_job_locally(config, mode)
        else:
            _schedule_job_on_slurm(config, mode)
...
def _schedule_job_on_slurm(cfg: Config, mode: str):
