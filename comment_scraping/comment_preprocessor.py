...
import numpy as np
import spacy
from comment_dataset_generator import create_substituions_datasets
...
def load_splits(preprocced_dir):
    dataset = {"train": {}, "val": {}, "test": {}}
    for split in dataset.keys():
        split_file_name = "final_recipe1m_" + split + ".pkl"
        split_file_name = os.path.join(preprocced_dir, split_file_name)
...
def read_ingredients(preprocessed_dir):
    ingredients = pickle.load(
        open(os.path.join(preprocessed_dir, "final_recipe1m_vocab_ingrs.pkl"), "rb")
    ).word2idx
    all_ingredients = []
    cleaned_ingredients = []
...
def preprocess_comments(recipe1m_path, preprocessed_dir):
    ...
    all_ingredients, cleaned_ingredients = read_ingredients(preprocessed_dir)
    ...
    train_ids, val_ids, test_ids = load_splits(preprocessed_dir)
