...
class Bert:
    def __init__(self, device, tokenizer=None, bert=None):
        ...
        if tokenizer:
            ...
            print("FoodBert Initiated")
        else:
            self.tokenizer = AutoTokenizer.from_pretrained("bert-base-uncased")
            print("Bert Initiated")
        if bert:
...
def get_bert_emb(titles, device, tokenizer=None, bert=None, batch_size=2000):
    ...
    index = 0
    embs = torch.zeros(len(titles), 768).to(device)
    with torch.no_grad():
...
def load_foodbert():
    ...
    model_args = ModelArguments(
        model_type="bert",
        model_name_or_path=default_dir + "foodbert/data/mlm_output4/checkpoint-770000",
    )
