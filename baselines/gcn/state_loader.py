...
def load_saved_models(output_dir, model, opt):
    ...
    try:
        ...
        print("*** NO SAVED MODEL FOUND in {}. LOADING FROM SCRATCH ****".format(output_dir))
        # Initilize the model
    ...
    try:
        ...
        print("*** Loading the model with the best MRR {} from epoch {} ****.".format(best_model.mrr.cpu().item(), best_model.epoch.cpu().item()))
    except:
...
def save_model(model, opt, output_dir, is_best_model=False):
    ...
    """
    if is_best_model:
        torch.save({"checkpoint": model.state_dict(),"epoch":model.epoch,"mrr":model.mrr}, os.path.join(output_dir, 'best_model.chkpnt'))
